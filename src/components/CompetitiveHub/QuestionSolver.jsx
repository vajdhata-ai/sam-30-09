...
const QuestionSolver = ({ examSlug, onBack }) => {
    ...
    if (selectedTopic && practiceLevel) {
        const questions = QUESTION_BANK[selectedTopic.id] || [];
        // Filter questions by level if we had real tags, for now mock subsetting
        const sessionQuestions = questions.slice(0, 5); // Simulate 5 questions per session

        if (sessionQuestions.length === 0) {
            return (
                <div className="h-full flex flex-col items-center justify-center p-8 bg-slate-950 text-center">
                    <p className="text-4xl mb-4">🚧</p>
                    <h2 className="text-xl font-bold text-white mb-2">Questions Not Available</h2>
                    <p className="text-slate-400 mb-6 max-w-md">The question bank for this specific topic and level is expanding. Please check back later.</p>
            ...
            <div className="h-full flex flex-col bg-slate-950">
                ...
                <div className="flex-shrink-0 p-4 border-b border-white/10 flex items-center justify-between bg-slate-900">
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-white text-sm font-bold mt-0.5 capitalize">{practiceLevel.replace('level', 'Level ')}</p>
                    </div>
                        ...
                        {isComplete ? (
                            <div className="text-center py-12 animate-in zoom-in duration-500">
                                ...
                                <p className="text-amber-400 font-black text-sm mb-8 uppercase tracking-widest">
                                    +{Object.values(answers).filter((a, i) => a === sessionQuestions[i].correctAnswer).length * 10} XP Earned
                                </p>
                            ...
                            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                                ...
                                <div className="p-6 md:p-8 rounded-3xl bg-slate-900 border border-white/5 mb-6 shadow-lg shadow-black/50">
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        {currentQ.tags && currentQ.tags.map(tag => (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <button onClick={() => setPracticeLevel('level1')} className="group text-left p-6 md:p-8 rounded-[2rem] bg-slate-900 border border-slate-800 hover:border-emerald-500/50 hover:bg-slate-800/80 transition-all shadow-xl shadow-black/20 hover:-translate-y-1">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <span className="px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-[9px] font-black uppercase tracking-widest border border-emerald-500/20">+30 XP</span>
                            </div>
                            <h3 className="text-2xl font-black text-white mb-2">Tier 1: Grunt</h3>
                            <p className="text-slate-400 text-sm font-medium mb-4">NCERT, Exemplar, and fundamental formula questions. Build your foundation.</p>
                        ...
                        <button onClick={() => setPracticeLevel('level2')} className="group text-left p-6 md:p-8 rounded-[2rem] bg-slate-900 border border-slate-800 hover:border-amber-500/50 hover:bg-slate-800/80 transition-all shadow-xl shadow-black/20 hover:-translate-y-1 relative overflow-hidden">
                            ...
                            <div className="flex items-center justify-between mb-6 relative z-10">
                                ...
                                <span className="px-3 py-1 rounded-full bg-amber-500/10 text-amber-400 text-[9px] font-black uppercase tracking-widest border border-amber-500/20">+50 XP</span>
                            </div>
                            <h3 className="text-2xl font-black text-white mb-2 relative z-10">Tier 2: Warrior</h3>
                            <p className="text-slate-400 text-sm font-medium mb-4 relative z-10">Intermediate difficulty matching {examSlug.toUpperCase()} Mains level.</p>
                        ...
                        {/* Level 3 */}
                        <button onClick={() => setPracticeLevel('level3')} className="group text-left p-6 md:p-8 rounded-[2rem] bg-slate-900 border border-slate-800 hover:border-red-500/50 hover:bg-slate-800/80 transition-all shadow-xl shadow-black/20 hover:-translate-y-1 md:col-span-2 lg:col-span-1 relative overflow-hidden">
                            <div className="flex items-center justify-between mb-6 z-10 relative">
                                ...
                                <span className="px-3 py-1 rounded-full bg-red-500/10 text-red-400 text-[9px] font-black uppercase tracking-widest border border-red-500/20">+75 XP</span>
                            </div>
                        ...
                        {/* PYQ Vault */}
                        <button onClick={() => setPracticeLevel('pyq')} className="group text-left p-6 md:p-8 rounded-[2rem] bg-gradient-to-br from-indigo-900 to-violet-900 border border-indigo-500/30 hover:border-indigo-400/80 transition-all shadow-2xl shadow-indigo-500/20 hover:-translate-y-1 hover:shadow-indigo-500/40 md:col-span-2 lg:col-span-1">
                            ...
                            <p className="text-indigo-200/80 text-sm font-medium mb-4">A massive archive of Past Year Questions categorized by this exact topic. Highly recommended.</p>
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-4 mt-4">
                                                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest flex items-center gap-1.5">
                                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Level 1
                                                </span>
                                                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest flex items-center gap-1.5">
                                                    <span className="w-1.5 h-1.5 rounded-full bg-amber-500"></span> Level 2
                                                </span>
                                            </div>
                                        </button>
                                    ))
